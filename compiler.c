...
static ObjFunction* endCompile(){
  ...
  #ifndef DEBUG_PRINT_CODE
  if(!parser.hadError){
    disassembleChunk(currentChunk(), function->name != NULL ? function->name->char : "<script>" );
  }
  #endif
...
static void forStatement() {
  ...
  if(!match(TOKEN_SEMICOLON)) {
    ...
    // Jump out of the loop if the codition is false.
  ...
  if(!match(TOKEN_RIGHT_PAREN)) {
    ...
    expression();
    emitByte(OP_POP);
    consume(TOKEN_RIGHT_PAREN, "Expect ')' after for clausers.");
